...
`fish-vision/src/annodb/storage_config.py`, fichier
...
def app_root() -> Path:
    ...
    # aquameasure-pyside/src/util/paths.py -> repo test/
    return Path(__file__).resolve().parents[3]


def _storage_config():
    """Configuration de stockage, ou `None` si `fish-vision` est indisponible.
...
def annotations_db_path() -> Path:
    ...
    return app_root() / "fish-vision" / "data" / "fish_annotations.db"
