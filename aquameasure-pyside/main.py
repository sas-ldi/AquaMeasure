...
def _setup_paths() -> Path:
    if getattr(sys, "frozen", False):
        ...
        app_candidates = (
            bundle / "aquameasure-pyside",
            repo / "aquameasure-pyside",
            bundle,
        )
        here = next(
        ...
        fv_candidates = (repo / "fish-vision", bundle / "fish-vision")
        fv_roots = [candidate for candidate in fv_candidates if candidate.is_dir()]
    else:
        here = Path(__file__).resolve().parent
        repo = here.parent
        fv_roots = [repo / "fish-vision"]

    # Namespace 'src' partagé : aquameasure-pyside/src + fish-vision/src (annodb).
