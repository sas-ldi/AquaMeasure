...
class CheminEcritureConcurrentTest(unittest.TestCase):
    ...
    def test_track_store_reste_pour_le_script_d_inference(self):
        store = REPO_ROOT / "annotations" / "src" / "track_store.py"
        self.assertTrue(store.is_file())
        script = REPO_ROOT / "annotations" / "scripts" / "infer_track_count.py"
...
class ComposantsQmlMortsTest(unittest.TestCase):
    ...
    def test_aucune_reference_qml_ni_qmldir(self):
        generated = QML_MIRROR / "qmldir"
        # Le miroir n'existe pas avant le premier lancement d'un clone neuf.
        qmldir = generated.read_text(encoding="utf-8") if generated.is_file() else ""
        for name in self.MORTS:
            with self.subTest(name=name):
...
class SauvegardeFusionneeTest(TempDirCase):
    ...
    def test_sauvegarde_et_export_utilisent_leurs_controleurs(self):
        roots = [QML_SRC] + ([QML_MIRROR] if QML_MIRROR.is_dir() else [])
        for root in roots:
            ...
            with self.subTest(root=root.name):
                self.assertIn("ProTools.exportSession(Sessions.selectedId)", exports)
...
class ColonnesDepreciesTest(unittest.TestCase):
    ...
    def test_aucune_lecture_de_is_grazing_sur_l_annotation(self):
        crops = (REPO_ROOT / "annotations/scripts/export_crops.py").read_text(
            encoding="utf-8",
        )
    ...
    def test_les_colonnes_restent_en_base_et_sont_documentees(self):
        ...
        models = (REPO_ROOT / "annotations/src/annodb/models.py").read_text(
            encoding="utf-8",
        )
