...
class ProToolsController(QObject):
    ...
    def _default_split_by(self) -> str:
        ...
        try:
            ...
            # Base absente, schéma antérieur, annotations non installé : le
    ...
    def _fv_root(self) -> Path:
        return paths.app_root() / "annotations"

    def _python_exe(self) -> str:
        if not getattr(sys, "frozen", False):
            venv = paths.app_root().parent / ".venv"
    ...
    def _run_script(
        ...
        fv = self._fv_root()
        if not fv.is_dir():
            self._logs.append("[!] Dossier annotations introuvable")
            if is_export:
