...
courant** (`interface/src/util/paths.py::app_data_root` renvoyait
...
pur `stdlib`, sans Qt et sans dépendance à `interface`, pour que la
...
`interface` → `annotations`, jamais l'inverse.
...
emplacements historiques (`annotations/data/fish_annotations.db`,
...
# annotations/src/annodb/storage_config.py -> annotations/
...
def default_db_path() -> Path:
    if getattr(sys, "frozen", False):
        return default_data_root() / "annotations" / "data" / DB_FILE_NAME
