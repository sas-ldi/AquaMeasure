...
(`.gitignore`, `annotations/models/*`). Historiquement ils etaient cherches
...
3. ``<depot>/annotations/models`` - emplacement historique, conserve pour ne
...
def _configured_data_root() -> Path | None:
    """Racine des donnees choisie par l'utilisateur, si `annotations` repond.
...
def model_search_dirs() -> tuple[Path, ...]:
    ...
    candidates.append(repo_root() / "annotations" / MODELS_SUBDIR)
