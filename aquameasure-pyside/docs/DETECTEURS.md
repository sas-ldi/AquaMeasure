...
```powershell
.\fish-vision\.venv\Scripts\python.exe fish-vision/scripts/download_fishial.py --pack classification
```

Les fichiers vont dans `fish-vision/models`. Relancer l’application, puis télécharger et activer le détecteur Fishial dans le gestionnaire. Les packs fournis avec un installateur et ceux d’une installation depuis les sources peuvent donc être différents.
...
Dans la version source, créer une sous-classe de DetectorBackend dans plugins/detectors. Elle doit charger son modèle et renvoyer des boîtes au contrat commun. Un exemple inactif est fourni dans ce dossier.
...
Les poids sont cherchés dans AQUAMEASURE_MODELS_DIR si défini, dans le dossier models de la racine de données configurée, puis dans les emplacements historiques de l'application. Les poids fournis par l'installeur se trouvent dans fish-vision/models à côté des ressources du programme.
