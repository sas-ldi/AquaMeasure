...
L'implémentation vit dans `fish-vision/src/annodb/storage_config.py` : c'est
...
est `aquameasure-pyside` → `fish-vision`, jamais l'inverse - dupliquer la
...
l'application (en s'assurant que `fish-vision` est sur le `sys.path`, comme le
...
def _ensure_fish_vision() -> None:
    ...
    # aquameasure-pyside/src/util/storage_config.py -> racine du dépôt
    repo = Path(__file__).resolve().parents[3]
    fv_root = repo / "fish-vision"
    fv_src = fv_root / "src"
