...
"""PyInstaller — AquaMeasure PySide6, distribution Windows portable.

Usage (depuis la racine du dépôt) :
    fish-vision\.venv\Scripts\python.exe -m PyInstaller --clean --noconfirm aquameasure-windows.spec
...
root = Path(SPECPATH)
app_dir = root / "aquameasure-pyside"
fv_dir = root / "fish-vision"
...
datas = []
datas += tree(app_dir / "qml", "aquameasure-pyside/qml")
datas += tree(app_dir / "resources", "aquameasure-pyside/resources")
datas += tree(app_dir / "src", "aquameasure-pyside/src", suffixes=(".py",))
for tool in ("__init__.py", "gen_qmldir.py", "release_models_test.py", "release_install_test.py"):
    path = app_dir / "tools" / tool
    if path.is_file():
        datas.append((str(path), "aquameasure-pyside/tools"))
datas += tree(fv_dir / "src", "fish-vision/src", suffixes=(".py",))
datas += tree(fv_dir / "scripts", "fish-vision/scripts", suffixes=(".py",))
datas += tree(fv_dir / "configs", "fish-vision/configs")
datas += tree(fv_dir / "db", "fish-vision/db")

# Base neuve au premier démarrage ; aucune donnée de travail n'est embarquée.
datas.append((str(fv_dir / "data" / ".gitkeep"), "fish-vision/data"))
...
for model_name in (
    ...
    if model.is_file():
        datas.append((str(model), "fish-vision/models"))
...
datas += tree(root / "vendor" / "yolov5", "vendor/yolov5")
...
datas += collect_data_files("fish_detectors")
...
a = Analysis(
    [str(app_dir / "main.py")],
    pathex=[str(app_dir), str(root), str(fv_dir), str(fv_dir / "src")],
    binaries=[],
    datas=datas,
